...
export default function Show({ attempt, quiz, questions }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (confirm('Are you sure you want to submit your quiz? You cannot modify your answers after submitting.')) {
            submitQuiz();
        }
    };
    ...
    return (
        <>
            ...
            <div className="p-6 max-w-7xl mx-auto space-y-6 w-full">
                ...
                <div className="sticky top-0 z-20 bg-white/80 dark:bg-neutral-900/80 backdrop-blur-md border border-neutral-200 dark:border-neutral-800 rounded-2xl p-4 shadow-sm flex items-center justify-between">
                    <div>
                        <h1 className="text-xl font-bold text-neutral-900 dark:text-white">{quiz.title}</h1>
                        <p className="text-xs text-neutral-500">Solve all questions before timer expires.</p>
                    </div>
                    <div className={`flex items-center gap-2 px-4 py-2 rounded-xl border ${
                        isTimeUrgent 
                            ? 'bg-red-50 border-red-200 text-red-700 animate-pulse dark:bg-red-950/20 dark:border-red-900/40 dark:text-red-400' 
                            : 'bg-brand-50 border-brand-100 text-brand-700 dark:bg-brand-950/20 dark:border-brand-900/40 dark:text-brand-400'
                    }`}>
                ...
                <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
                    {questions.map((question, index) => {
                        const currentAnswer = data.answers.find(a => a.question_id === question.id)?.answer_text || '';

                        return (
                            <div key={question.id} className="rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-6 shadow-xs space-y-4">
                                            ...
                                            return (
                                                ...
                                                >
                                                    <input
                                                        ...
                                                        className="size-4 text-brand-500 focus:ring-brand-500 border-neutral-300"
                                                    />
                                                    <span className={`text-sm font-medium ${isSelected ? 'text-brand-900 dark:text-white' : 'text-neutral-700 dark:text-neutral-300'}`}>
                                                        {opt}
                                                    </span>
                                                </label>
                                            );
                    ...
                    <div className="flex items-center justify-end gap-3 pt-4">
                        <Button type="submit" disabled={processing} className="flex items-center gap-2 px-6">
